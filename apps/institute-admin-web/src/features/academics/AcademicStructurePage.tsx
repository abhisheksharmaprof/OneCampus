...
function TeacherMapping({ accessToken, branchId, subjects, curriculum, classes }: { accessToken: string; branchId?: string; subjects: Subject[]; curriculum: ClassSubject[]; classes: AcademicClass[] }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const detail = cause instanceof AcademicsApiError ? Object.values(cause.fieldErrors).flat().join(' ') : ''
      setError(detail || (cause instanceof Error ? cause.message : 'Teacher mapping could not be saved.'))
  ...
  const changeClass = (value: string) => { setError(''); setClassId(value); setSectionIds([]); setCombinedSlot(''); setSubjectId('') }
  return <section className="class-mapping-panel"><div className="class-mapping-toolbar"><div><strong>Teacher mapping</strong><p>Select a class, then a section when sections are configured, followed by a mapped subject and teacher.</p></div><div className="class-mapping-controls"><select aria-label="Mapping class" value={classId} onChange={(event) => changeClass(event.target.value)}><option value="">Select class</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{classSections.length > 0 && <fieldset className="section-multiselect" aria-label="Mapping sections"><legend>Sections</legend><label><input type="checkbox" checked={allSelected} onChange={toggleAll} /> All sections</label>{classSections.map((section) => <label key={section.id}><input type="checkbox" checked={sectionIds.includes(section.id)} onChange={() => toggleSection(section.id)} /> {section.sectionName}</label>)}</fieldset>}{sectionIds.length > 1 && <input aria-label="Combined slot label" placeholder="Combined slot (optional, e.g. Second Language)" value={combinedSlot} onChange={(event) => setCombinedSlot(event.target.value)} maxLength={120} />}<select aria-label="Mapping subject" value={subjectId} disabled={!classId} onChange={(event) => setSubjectId(event.target.value)}><option value="">{classId ? (availableSubjects.length ? 'Select subject' : 'No subjects mapped') : 'Select class first'}</option>{availableSubjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select><select aria-label="Mapping teacher" value={teacherId} onChange={(event) => setTeacherId(event.target.value)}><option value="">Select teacher</option>{teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.fullName}</option>)}</select><button className="button-primary btn-sm" type="button" disabled={busy} onClick={() => void save()}><Plus size={14} /> Map teacher</button></div></div>{error && <p className="form-error" role="alert">{error}</p>}<div className="subject-table-wrap"><table className="subject-reference-table"><thead><tr><th>Class section</th><th>Subject</th><th>Teacher</th><th>Actions</th></tr></thead><tbody>{assignments.map((assignment) => <tr key={assignment.id}><td>{assignment.classSectionLabel ?? assignment.classSectionId}{assignment.combinedSlotLabel && <small className="status-badge tone-info">{assignment.combinedSlotLabel}</small>}</td><td><strong>{assignment.subject.name}</strong><small>{assignment.subject.subjectCode}</small></td><td>{assignment.teacher.fullName}</td><td><button className="button-secondary btn-sm danger-text" type="button" disabled={busy} onClick={() => void remove(assignment)}>Remove</button></td></tr>)}</tbody></table>{!assignments.length && <p className="subject-empty">No teacher mappings yet. Select a class, section if available, subject, and teacher above.</p>}</div><small className="section-caption">Showing {assignments.length} teacher mapping{assignments.length === 1 ? '' : 's'}.</small></section>
}
